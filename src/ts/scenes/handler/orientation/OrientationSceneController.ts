import { DataProps, EventNames, OrientationSceneView } from "./OrientationSceneView";
...
export class OrientationSceneController extends Phaser.Scene {
	...
	init (): void {
		...
		this.onInitializeOrientation(() => {
			this.registry.set(DataProps.initialize, true);
			this.registry.set(DataProps.initOnLandscape, this.isLandscape());
		...
		this.onOrientationWarningHide(() => {
			const isInitOnLandscape = this.registry.get(DataProps.initOnLandscape);
	...
	isLandscape (): boolean {
		if (/Windows/i.test(navigator.userAgent)) return false; // NOTE: Should be remove on production
	...
	update (time: number, dt: number): void {
		if (this.view.isInitialized) return;
		this.view.event.emit(EventNames.onInitializeOrientation);
	}
